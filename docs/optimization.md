...
> 姊妹文档：**功能性需求 + 商业化升级**见 [monetization.md](monetization.md)。
...
### Q4 · 数据上云策略决策 ✅ 已决策（2026-05-12）
- [x] 决策：**sessions / cards 上云**，作为付费层的核心价值之一
- [ ] 实现工作：见 [monetization.md M4（跨设备同步）](monetization.md#m4--跨设备同步sessions--cards-复杂度最高)
- **背景**：当前 [backend/drizzle/0000_wise_magik.sql](../backend/drizzle/0000_wise_magik.sql) 只有 `users` + `refresh_tokens`，会话数据全在客户端 SQLite。
- **决策依据**：换手机 / 重装即丢——直接做成"付费版的差异化价值"，免费用户继续本地存储、付费用户云端同步。一个决策同时解决"用户投诉"和"付费理由"两件事
