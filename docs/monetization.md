...
| 套餐 | 价格 | 说明 |
|------|------|------|
| 免费 | ¥0 | 每月 5 次 session，follow-up 3 次/session，本地存储 |
| 月订阅 | ¥9.9/月 | 无限 session，无限 follow-up，云端音频存储 |
| 年订阅 | ¥88/年 | 同上，折合 ¥7.3/月 |

苹果内购扣完 30% 实际到手：月订阅 ¥6.9，年订阅 ¥61.6。
...
上线时给现有 20 个内测用户发专属优惠码：首月 ¥6.9 或年付 ¥68，感谢参与内测。转化率预期远高于冷启动获客。
...
### M1 · 苹果内购 + 订阅状态 ⭐ 必做基础
- [ ] 客户端：集成 IAP（[react-native-iap](https://github.com/dooboolab-community/react-native-iap) 或 expo-modules + StoreKit 2 桥接）
- [ ] App Store Connect：创建月订阅 + 年订阅两个 product；配置 Subscription Group + Subscription Levels
- [ ] 后端：`POST /auth/iap/verify` — 接收客户端上报的 receipt → 调 Apple `verifyReceipt` API → 解出订阅有效期 → 写入 DB
- [ ] DB schema 加字段：`users.subscription_status` (`'free' | 'monthly' | 'yearly'`)、`subscription_expires_at`、`subscription_original_transaction_id`
- [ ] Apple Server-to-Server Notifications（订阅续费、取消、退款）→ webhook 端点 `POST /webhooks/apple/subscription`
- [ ] 客户端轮询订阅状态（启动时 + 进 paywall 前），从后端拿权威值，不信任本地缓存
- **难点**：StoreKit 沙盒环境跟生产环境是分开的，receipt 验证逻辑要双 endpoint 兼容。Apple webhook 设置需要在 App Store Connect 配 production / sandbox 两个 URL。
- **参考**：Apple [In-App Purchase server-side validation](https://developer.apple.com/documentation/appstoreservernotifications)
...
- [ ] follow-up 一样 check：当前 session 的 followup ≥ 3 → 拦截
...
- [ ] 一次性迁移脚本：现有 20 个用户本地的 WAV 文件批量上传到 OSS、回填 `sentence_audio_uris` 为 OSS key
...
- [ ] **或者**：用 Apple [Offer Codes](https://developer.apple.com/app-store/subscriptions/) 功能创建"首月 ¥6.9"和"年付 ¥68" 两档促销；通过 URL `https://apps.apple.com/redeem?ctx=offercodes&id=APP_ID&code=CODE` 分发
...
| 用户录音（WAV）| OSS Standard | ~2MB / session | M3 上传，本地保留 LRU 缓存 |
...
- **月成本约 ¥40-60**，相对 1000 用户的订阅收入（保守 30% 是月订阅 = 300 × ¥6.9 = ¥2070/月）忽略不计
...
**关键提示**：本文档的 M3 + M4 是 optimization.md 里 P3/P4/P12/P24 几个长期项的**强催化**——付费版上线就必须有云端存储 + 同步，所以 Phase β 实际上同时推进了这两个文档的进度。
...
- 选项 C：永久保留，免费用户只是不能新创建
- 建议：**C**——存储成本低（旧用户量级有限），保留数据降低用户离开摩擦
